...
(function () {
    ...
    var size_RegExp = /^([\d\.]+)\s([a-zA-Z]?)B(?:\s\(([\d\.]+)%\))?$/;
    var ratio_RegExp = /^(∞|\-\-|[\d\.]+)$/;
    ...
    function f_duration(duration) {
        return Math.pow(2, duration / (24 * days_per_year));
    }
    // The size factor for yen generation
    ...
    function yen_generation_title(size, seeders, duration) {
        ...
        title += '¥' + (time_frame * f_size(size)).toPrecision(6) + ' \tbase for size';
        ...
        temp = (100 * (f_duration(duration) - 1)).toFixed(1);
        if (temp !== '0.0') {
            title += '\n+' + temp + '% \tfor your seeding time';
        }
        temp = (100 * (f_seeders(seeders) - 1)).toFixed(1);
        if (temp !== '0.0') {
            title += '\n' + temp + '% \tfor the number of seeders';
        }
        title += '\n\n¥ per ' + time_frame_string + ' \t¥ per ' + time_frame_string + ' per GB\t#seeders\n';
        var start = Math.max(seeders - 1, 5);
        var end = Math.max(seeders + 1, 5);
            ...
            for (var i = 0, length = real_torrents.length; i < length; i++) {
                ...
                if (to_be_collapsed) {
                    ...
                    if (corresponding !== null) {
                        corresponding.style.visibility = 'collapse';
                    }
                }
                else {
                    torrent.style.visibility = '';
                    if (corresponding !== null) {
                        corresponding.style.visibility = '';
                    }
                }
            }
    ...
    if (show_yen && GM_getValue('creation', '0').toString() === '0') {
        var user_id = document.querySelector('div#header div#userinfo li#username_menu a.username');
        if (user_id !== null && user_id.href === document.URL) {
            ...
            for (var i = 0, length = children.length; i < length; i++) {
                ...
                if (child.textContent.indexOf("Join") !== -1) {
                    try {
                        ...
                        GM_setValue('creation', JSON.stringify(Date.parse(join_date)));
                    }
                    catch (error) { }
                }
            }
