...
(function() {
  var a1, a2, allRows, andRe, clonedNode, colorRows, curPage, currencyRe, dateTimeRe, downIndex, downRe, dur2string, durIndex, durationRe, dynamicLoad, header, headers, i, index, j, lastPage, lcNegBG, lcNegFG, lcNeuBG, lcNeuFG, lcPosBG, lcPosFG, len, len1, line_color_neg, line_color_neu, line_color_pos, loadPage, multiRatio, newPagenum, nextPage, pagenum, pagenums, parseCell, parseRows, prevPage, ratioIndex, ratioRe, sizeIndex, sizeRe, sortFunctions, sortIndex, sortRows, unitPrefix;
    ...
    for (i = 0, len = torrent_rows.length; i < len; i++) {
      ...
      myData = (function() {
        ...
        for (index = j = 0, len1 = ref.length; j < len1; index = ++j) {
          cell = ref[index];
          results.push(parseCell(cell, index));
        }
        return results;
      })();
      completion = 100;
      ...
      line_color = ['', ''];
      if (downIndex != null) {
        ref = myData[downIndex], downloaded = ref[0], completion = ref[1];
        if (completion > 0) {
  ...
  if (sortRows.toString() === 'true') {
    for (index = i = 0, len = headers.length; i < len; index = ++i) {
  ...
  if (dynamicLoad.toString() === 'true') {
    ...
    for (j = 0, len1 = pagenums.length; j < len1; j++) {
      pagenum = pagenums[j];
